...
export class ToolsController {
  ...
  @Get()
  getAllTools(@Query() { limit, offset }: ToolsPaginationDto) {
  ...
  async getAllByKeywords(
    ...
    const response = await this.toolsService.getAllByKeywords(
      search.keywords,
      limit,
      offset,
    );
  ...
  @Get(
    '/herramientas/:categoryId/:categoryName/:subcategoryId/:subcategoryName/p/:toolId/:toolName',
  )
  ...
  async getToolsByCategoryId(@Param('categoryId') categoryId: number) {
    ...
    if (!tools)
      throw new NotFoundException(
        'Error: No tools found with the provided categoryId.',
      );
    return tools;
  }
  ...
  async getToolsBySubcategoryId(@Param('subcategoryId') subcategoryId: number) {
    if (isNaN(subcategoryId))
      throw new NotFoundException('Id is not a number.');
    const tools =
      await this.toolsService.getToolsBySubcategoryId(subcategoryId);
    if (!tools)
      throw new NotFoundException(
        'Error: No tools found with the provided subcategoryId.',
      );
    return tools;
  }
    ...
    try {
      ...
      if (error.code === 11000) {
        throw new ConflictException(
          `Duplication error, Tool ${error} already exists!`,
        );
      }
      throw error;
    }
